    ...
    <table class="crud-table" data-max-id="{{ $User::max('id') }}"
        ...
        @foreach (array_merge($paginator->items(), $filler_rows) as $user)
            <tr data-row-id="{{ $user->id }}">
                ...
                <td><input type="text" value="{{ $user->last_name }}" @if($is_exist_user) autocomplete="new-password" disabled @endif onfocusout="update_cell_of(this)"></td>
                <td><x-crud-components.role-select :$user /></td>

                <td><input type="password" value="{{ $is_exist_user ? '***' : '' }}" @if($is_exist_user) autocomplete="new-password" disabled @endif onfocusout="update_cell_of(this)"></td>
                ...
                <td>
                    @can('delete ' . ("{$user->getRoleNames()->first()}" ?: 'user')) <!-- ?: 'user' guarantied that EmptyRow object users will be deletable -->
